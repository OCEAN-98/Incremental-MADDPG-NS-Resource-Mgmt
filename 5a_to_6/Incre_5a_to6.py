...
load_dir = '/Users/ocean/git/NetworkSlicing/4to5_slices/models'
...
models = [agent_0_actor, agent_1_actor, agent_2_actor, agent_3_actor, agent_4_actor]
...
models = [agent_0_critic, agent_1_critic, agent_2_critic, agent_3_critic, agent_4_critic]
...
'''weights = [] # a list to record weights and bias

for model in models:
    model_weights = model.state_dict()
    weights.append(model_weights)
    for key in model_weights.keys():
        model_weight = model_weights[key].clone().detach()
        weights.append(model_weight)

avg_weights = torch.mean(torch.stack(weights), dim=0)

agent_5_actor = ActorNetwork(alpha, actor_input, conv1, conv2, n_actions, chkpt_dir=chkpt_dir, load_dir=load_dir, name= 'agent_5_actor') # only use save
agent_5_actor.load_state_dirc({k: avg_weights[i] for i, k in enumerate(agent_5_actor.state_dict().keys())})
agent_5_actor.save()

models = [agent_1_critic, agent_2_critic, agent_3_critic, agent_4_critic]
weights = [] # a list to record weights and bias
...
agent_5_critic = CriticNetwork(alpha, actor_input, conv1, conv2, n_actions, chkpt_dir=chkpt_dir, load_dir=load_dir, name= 'agent_5_critic')
agent_5_critic.load_state_dirc({k: avg_weights[i] for i, k in enumerate(agent_5_critic.state_dict().keys())})
agent_5_critic.save()
'''